...
const CreateTransferForm = () => {
  ...
  const handleUpdate = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8080/api/v1/locations/${formData.fromLocation}?orderId=${formData.orderId}`,
        {
          method: "GET",
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`http://localhost:8080/api/v1/deliveries`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: "Bearer " + localStorage.getItem("accessToken"),
      ...
      if (response.ok) {
        ...
        setDialogMessage(`Mã vận chuyển: ${data?.id}`);
      } else {
        const msg = data.error;
        setDialogTitle("Thất bại");
