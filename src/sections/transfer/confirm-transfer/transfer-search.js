...
const TransferSearch = () => {
  ...
  const fetchTransferById = async (transferId) => {
    ...
    const response = await fetch(`http://localhost:8080/api/v1/deliveries/${transferId}`, {
      method: "GET",
      headers: {
          ...
          <Paper elevation={3} sx={{ padding: 4, margin: [4, 4, 4, 4], boxShadow: 3 }}>
            <Grid container spacing={3}>
              <Grid item xs={12} md={12}>
                ...
                <Typography variant="body2" color="textSecondary">
                  Điểm vận chuyển {transfer.presentDes}
                </Typography>
                <Typography variant="body2" color="textSecondary">
                  Điểm nhận hàng: {transfer.nextDes}
                </Typography>
                <Typography variant="body2" color="textSecondary">
                  Thời điểm vận chuyển: {new Date(transfer.dateCreated).toLocaleString()}
                </Typography>
                {/* <Typography variant="body2" color="textSecondary">
